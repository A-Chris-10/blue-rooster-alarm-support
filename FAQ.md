---
layout: page
title: FAQ and Feedback
---

## Support hours & response
We typically respond as soon as possible. If your alarm issue is urgent, please include **“URGENT”** in the ticket subject and share the troubleshooting steps you already tried.

---

## Safety note (important)
If you use Blue Rooster Alarm for medical, safety, or mission-critical needs, consider setting a **backup alarm** on your device or a secondary device.



# Feature Requests & Feedback

We use feedback to prioritize what we build next.

## What to include
- The problem you’re trying to solve
- What you expected Blue Rooster Alarm to do
- Your ideal workflow (step-by-step)
- Platform: iOS / Android / Web
- Screenshots or quick mockups (optional)

Submit Feature Requests & Feedback Here:
---
<script src="https://desk.zoho.com/portal/api/feedbackwidget/1111893000000396005?orgId=911803023&displayType=embeded"></script>
<div id="zsfeedbackwidgetdiv"></div> 


